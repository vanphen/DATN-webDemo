...
@section('content')
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="panel-body">
              <div class="table-responsive">
                <table class="table table-hover">
                  ...
                  @foreach(Cart::content() as $row)
                    <tr>
                      <td>{!!$row->id}}</td>
                      <td><img src="{{ asset('uploads/products/'.$row->options->img)}}" alt="dell" width="80" height="50"></td>
                      <td>{!!$row->name}}</td>
                      <td class="text-center">                        
                          @if (($row->qty) >1)
                          <a href="{{ asset('gio-hang/update/'.$row->rowId.'/'.$row->qty.'-down')}}"><span class="glyphicon glyphicon-minus"></span></a> 
                          @else
                        ...
                        <a href="{{ asset('gio-hang/update/'.$row->rowId.'/'.$row->qty.'-up')}}"><span class="glyphicon glyphicon-plus-sign"></span></a>
                      </td>
                      <td><a href="{{ asset('gio-hang/delete/'.$row->rowId)}}" onclick="return xacnhan('Xóa sản phẩm này ?')" ><span class="glyphicon glyphicon-remove" style="padding:5px; font-size:18px; color:red;"></span></a></td>
              ...
              @if(Cart::count() !=0)
                @if (Auth::guest())
                  ...
                  <a class="btn btn-large btn-warning pull-right" href="{{ asset('/login')}}" >Tiến hàng thanh toán</a>
                @else
                  <form action="{{ asset('/dat-hang')}}" method="get" accept-charset="utf-8">                    
                    <div class="input-group">
                    ...
                    <hr>
                      ...
                      <a href="{{ asset('/')}}" type="button" class="btn btn-large btn-primary pull-left">Tiếp tục mua hàng</a>
                  </form>
                @endif
              @endif
              </div>
            ...
            <div class="thumbnail mobile">              
              <div class="bt">
                <div class="image-m pull-left">
                  <img class="img-responsive" src="{{ asset('uploads/products/'.$row->images)}}" alt="{!!$row->name}}">
                </div>
              ...
              <div class="ct">
                <a href="{{ asset('mobile/'.$row->id.'-'.$row->slug)}}" title="Chi tiết">
                ...
                <a href="{{ asset('gio-hang/addcart/'.$row->id)}}" class="btn btn-success pull-right add">Thêm vào giỏ </a>
            </div> <!-- / div thumbnail -->
          </div>  <!-- /div col-4 -->
        @endforeach        
    ...
    <div class="panel panel-info">
      ...
      <div class="panel-body no-padding">
        <a href="#" title=""><img src="{{ asset('images/slides/thumbs/qc1.png')}}" alt="" width="100%" height="100%"> </a> <br>
        <a href="#" title=""><img src="{{ asset('images/slides/thumbs/qc2.png')}}" alt="" width="100%" height="100%"> </a> <br>
