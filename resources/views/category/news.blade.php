        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
          <div class="panel panel-success">
            <div class="panel-body">
              ...
              <!-- hot new content -->              
                <div class="col-lg-6">
                  <img src="{{ asset('uploads/news/'.$hot1->images)}}" alt="" height="200" width="100%">
                  <h3 class="title-h3"><a href="{{ asset('tin-tuc/'.$hot1->id.'-'.$hot1->slug)}}" title="">{!!$hot1->title}} </a></h3>
                ...
                <div class="col-lg-6 no-padding">
                  <div class="row">
                    @foreach($data as $row)
                      <div class="col-lg-12 ">
                        <h4><a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title="{!!$row->title}}">{!!$row->title}}</a></h4>
                        <div class="col-lg-9">
                          <p class="sum">{!!$row->intro}}</p>
                        </div>
                        <div class="col-lg-3 no-padding">
                          <a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title=""><img src="{{ asset('uploads/news/'.$row->images)}}" alt="" width="80" height="80" style="padding-right:10px; padding-left: 0;"></a>
                        </div>
                      </div>
                    @endforeach                   
                  </div>                                     
              ...
              <div class="row">
                @foreach($all as $row)
                  <div class="col-lg-12 no-padding">
                    <hr>
                    <div class="col-lg-3">
                      <a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title="{!!$row->slug}}"><img src="{{ asset('/uploads/news/'.$row->images)}}" alt="" width="90%" height="99%"> </a>
                    </div>
                    <div class="col-lg-9">
                      <h4><a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title="">{!!$row->title}}</a></h4>
                      <p> 
                        {!!$row->intro}}
                      </p>
            ...
            <div class="tab-pane active" id="1">
              <ul class="li-menu-tab">
                ...
                @foreach($all as $row)
                  <div class="col-lg-12 no-padding">
                    <div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 no-padding">
                      <a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title="{!!$row->title}}"><img src="{{ asset('uploads/news/'.$row->images)}}" alt="{!!$row->images}}" width="99%" height="99%"> </a>
                    </div>
                    <div class="col-xs-12 col-sm-8 col-md-8 col-lg-8 no-padding">
                     <a href="{{ asset('/tin-tuc/'.$row->id.'-'.$row->slug)}}" title="">{!!$row->title}}</a>
                    </div>
                  </div>
                @endforeach                  
